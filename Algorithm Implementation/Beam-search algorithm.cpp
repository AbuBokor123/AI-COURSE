...
int main()
{
    ...
    while (!q.empty())
    {
        vector<int> next_level; // init vec
        while (!q.empty())
        {
            int node = q.front();
            q.pop();
            ...
            next_level.insert(next_level.end(), graph[node].begin(), graph[node].end()); /// store all nodes that expoled in the next level
        }
      
       
        sort(next_level.begin(), next_level.end(), [&](int a, int b)  
             { return heuristic[a] < heuristic[b]; }); /// after getting the all neighbours sort them on the basis of heuristic values

        q = queue<int>(); /// after sorting them then clear the queue
